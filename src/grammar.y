...
rule
  ...
  # Method definition
  Def:
    MATEY IDENTIFIER Block          { result = DefNode.new(val[1], [], val[2]) }
  | MATEY IDENTIFIER
  ...
  # Class definition
  Class:
    SHIP CONSTANT Block          { result = ClassNode.new(val[1], val[2]) }
  ;
  ...
  # lexer.
  Block:
    Terminator Expressions ENDS       { result = val[1] }
  | Terminator ENDS                   { result = [] }
